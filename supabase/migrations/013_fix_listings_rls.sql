...
-- Create clean, comprehensive policies using CREATE OR REPLACE for safety
CREATE OR REPLACE POLICY "Allow anonymous listing creation" ON listings
  FOR INSERT WITH CHECK (true);

CREATE OR REPLACE POLICY "Anyone can view active listings" ON listings
  FOR SELECT USING (status = 'active');

CREATE OR REPLACE POLICY "Users can view their own listings" ON listings
...
CREATE OR REPLACE POLICY "Admins can manage all listings" ON listings
  FOR ALL USING (
    EXISTS (
